...
def linreg(X, y, reg=0.0):
	...
	"*** YOUR CODE HERE ***"
	I = np.eye(X.shape[1])
	I[0, 0] = 0 # checked answer key for the bias term
...
def RMSE_vs_lambda(X_train, y_train, X_val, y_val):
	...
	W_list = []
	"*** YOUR CODE HERE ***"
	reg_list = sorted(np.random.uniform(0,150.0, 150))
	W_list = list(map(lambda regparam: linreg(X_train, y_train, reg = regparam), reg_list))
...
def norm_vs_lambda(X_train, y_train, X_val, y_val):
	...
	norm_list = []
	"*** YOUR CODE HERE ***"
	reg_list = sorted(np.random.uniform(0,150, 150))
	W_list = list(map(lambda reg: linreg(X_train, y_train, reg), reg_list))
...
def linreg_no_bias(X, y, reg=0.0):
	...
	"*** YOUR CODE HERE ***"
	I = np.eye(X.shape[0])
	agg = X.T @ (I - np.ones(X.shape[0])/X.shape[0])# checked answer key for the no bias term
	W_opt = np.linalg.solve(agg @ X + reg * np.eye(agg.shape[0]), agg @ y)	
...
def grad_descent(X_train, y_train, X_val, y_val, reg=0.0, lr_W=2.5e-12, \
	...
	"*** YOUR CODE HERE ***"
	W, b = np.zeros((n,1)), 0
	W_grad, b_grad = np.inf*np.ones_like(W), np.inf
	...
	while np.linalg.norm(W_grad) > eps and np.linalg.norm(b_grad) > eps \
		...
		"*** YOUR CODE HERE ***"
		obj_train += [np.sqrt(np.linalg.norm((X_train @ W).reshape((-1, 1)) + b - y_train)**2/m_train)]
		obj_val += [np.sqrt(np.linalg.norm((X_val @ W).reshape((-1,1)) + b - y_val)**2 / m_val)]
